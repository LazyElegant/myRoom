...
public class StudentForgotPassword {
	public static void App() {
		System.out.println("欢迎进入学生找回密码界面！");
		// 获取学号
		...
		String sql = "SELECT code FROM student WHERE CODE = '"+code+"' AND tel ='"+tel+"'";
		...
		try {
			if(result.next()) {
				...
				// 调用数据库，更改密码
				String sql1 = "UPDATE student SET password = '"+MD5Util.getMD5(password.getBytes())+"' WHERE code='"+code+"'";
				...
				System.err.println("用户不存在，请查看学号或手机号是否正确！");
