				...
				<div class="clearfix">
					...
					<c:if test="${number>=5}">
						...
						<div class="page-inner">
							<c:if test="${page - 1>0}">
								<a href="/listByClass?id=${id}&page=${page-1}" class="n">&lt; 上一页</a>
							</c:if>
							<c:forEach var="i" begin="1" end="${page-1}" step="1">
								<a href="/listByClass?id=${id}&page=${i}"><span class="pc">${i}</span></a>
							</c:forEach>
							...
							<a href="/listByClass?id=${id}&page=${page+1}" class="n">下一页 &gt;</a>
							</c:if>
						</div>
					</c:if>
				</div>
