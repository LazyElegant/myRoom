				...
				<div class="mauto" style="width: 1166px;padding-bottom: 20px;">
					<div class="clearfix">
						<ul id="left_nav" class="fl-left bgfff">
							...
							<li class="c_menu">
								<a href="javascript:;" id="grzx" class="choosed">个人中心</a>
							</li>
							<li class="c_menu">
						...
						<div id="Model" class="fl-right">
							<div id="content2" class="thiscontent" style="background-color: #fff;border-top: 3px solid #8B3734;padding: 20px;padding-bottom:0;min-height: 209px;">
								<p style="font-weight: bold;font-size: 13px;">个人中心</p>
								<form action="/updateUser" method="post" style="width: 600px;height: 200px;margin: 40px 40px;font-size: 18px;">
									id: <br /><input value="${sessionScope.user.getId()}" class="num" name="id" onfocus=this.blur() type="text" style="width: 300px;font-size: 18px;padding: 2px;margin: 2px;" /><br />
									用户名: <br /><input value="${sessionScope.user.getUsername()}" name="username" type="text" style="width: 300px;font-size: 18px;padding: 2px;margin: 2px;" /><br />
									邮箱: <br /><input value="${sessionScope.user.getEmail()}" name="email" type="text" style="width: 300px;font-size: 18px;padding: 2px;margin: 2px;" /><br />
									权限等级: <br /><input value="${sessionScope.user.getStatus()==1 ? '管理员':'用户'}" class="status" onfocus=this.blur() type="text" style="width: 300px;font-size: 18px;padding: 2px;margin: 2px;" /><br />
	...
	<script>
		...
		function cc(ca){
			if(ca.next()[0].tagName != document.createElement('span').tagName){
				var span = ca.after("<span style='color:red'>该项不可以修改</span>");
				ca.next()
